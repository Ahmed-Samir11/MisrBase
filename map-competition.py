...
def save_category_model(model, tokenizer, encoder, output_dir="output", model_name="category_model"):
    ...
    try:
        ...
        if hasattr(tokenizer, 'save_pretrained'):
            ...
            print(f"✅ Category tokenizer saved to: {tokenizer_path}")
        else:
        ...
        print(f"✅ Category metadata saved to: {metadata_path}")
        
        print(f"\n🎉 Category model successfully saved with timestamp: {timestamp}")
        return saved_files
        
    except Exception as e:
        print(f"❌ Error saving category model: {str(e)}")
        return None
...
def save_misconception_model(model, tokenizer, encoder, output_dir="output", model_name="misconception_model"):
    ...
    try:
        ...
        if hasattr(model, 'save_pretrained'):
            # For Hugging Face models
            model.save_pretrained(model_path)
            print(f"✅ Misconception model saved to: {model_path}")
        else:
            # For other models (sklearn, etc.)
            model_file = f"{model_path}.joblib"
            joblib.dump(model, model_file)
        ...
        if hasattr(tokenizer, 'save_pretrained'):
            tokenizer.save_pretrained(tokenizer_path)
            print(f"✅ Misconception tokenizer saved to: {tokenizer_path}")
        else:
            tokenizer_file = f"{tokenizer_path}.joblib"
            joblib.dump(tokenizer, tokenizer_file)
            tokenizer_path = tokenizer_file
        ...
        joblib.dump(encoder, encoder_path)
        saved_files['encoder'] = encoder_path
        print(f"✅ Misconception encoder saved to: {encoder_path}")
        ...
        with open(metadata_path, 'w') as f:
            json.dump(metadata, f, indent=2)
        saved_files['metadata'] = metadata_path
        print(f"✅ Misconception metadata saved to: {metadata_path}")
        
        print(f"\n🎉 Misconception model successfully saved with timestamp: {timestamp}")
        return saved_files
        
    except Exception as e:
        print(f"❌ Error saving misconception model: {str(e)}")
        return None
...
print("🚀 Student Explanation Classification with MAP@3")
print("=" * 60)
